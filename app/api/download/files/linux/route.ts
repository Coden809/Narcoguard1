...
export async function GET(request: Request) {
  try {
    ...
    try {
      ...
      if (!tokenData || (tokenData.platform !== "linux" && tokenData.platform !== "desktop")) {
    ...
    // Path to the Linux AppImage file
    const filePath =
      process.env.LINUX_APPIMAGE_PATH || path.join(process.cwd(), "public", "downloads", "narcoguard.AppImage")
    ...
    if (!fs.existsSync(filePath)) {
      ...
      const sampleContent = `This is a sample Linux AppImage file for Narcoguard.
...
This would be the actual application binary in production.`
    ...
    return new NextResponse(fileBuffer, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="narcoguard.AppImage"`,
